...
{		
	...
	public:
		...
		{
			vector<shared_ptr<HornClause>> v;
			Parser parser("( (a) )");
			v.push_back(parser.parse_horn_clause());

			for (shared_ptr<HornClause> i : v) {
				i->fill_symbol_table();
			}
			string predicate_query = "(a)";

			ostringstream s;
			Assert::AreEqual(0, evaluate(predicate_query,  v, s));
		...
		TEST_METHOD(evaluate_book_ancestor_fail)
		{
			...
			ostringstream s;
			Assert::AreEqual(1, evaluate(predicate_query,  v, s));
			ostringstream result; 
		...
		TEST_METHOD(evaluate_book_loop)
		{
			...
			ostringstream s;
			Assert::AreEqual(2, evaluate(predicate_query,  v, s));
			ostringstream result; 
